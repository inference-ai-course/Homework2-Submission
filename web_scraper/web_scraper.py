...
class ArxivScraper:
    ...
    def scrape_article_content(self, article_url):
        ...
        try:
            ...
            arxiv_id = article_url.split('/abs/')[-1] if '/abs/' in article_url else "Unknown ID"
            
            # Get title, authors, and description from metadata or content
            title = metadata.title if metadata and metadata.title else "Unknown Title"
            authors = metadata.author if metadata and metadata.author else "Unknown Authors"
            description = metadata.description if metadata and metadata.description else ""
            
            # Try to extract abstract from the raw content if not in metadata
            abstract = description
            if not abstract and extracted_content:
                # Look for abstract in the extracted content
                abstract_match = re.search(r'Abstract[:\s]+(.*?)(?:\n\n|\n[A-Z])', extracted_content, re.IGNORECASE | re.DOTALL)
                if abstract_match:
            ...
            logger.info(f"Successfully scraped article: {arxiv_id}")
            return article_data
...
def main():
    ...
    list_urls = [
        ...
        "https://arxiv.org/list/cs/recent?skip=100&show=100"
    ]
    ...
    # Scrape articles
    articles = scraper.scrape_articles_from_lists(list_urls, 'arxiv_articles.json')
    
    # Print summary
    print(f"\n=== Scraping Summary ===")
    print(f"Total articles scraped: {len(articles)}")
    print(f"Results saved to: arxiv_articles.json")
